...
namespace kaleidoscope {
  namespace ast {
    ...
    {
    public:
      BinaryOp(std::string op, Exp* right, Exp* left) : op_(), right_(right), left_(left) {}
      virtual ~BinaryOp()
      {
        delete right_;
        delete left_;
      ...
      static sphingid::ast::Node* make(std::vector<sphingid::ast::Node*> v)
      {
        ...
        assert(dynamic_cast<Exp*>(v[1]));
        assert(dynamic_cast<Exp*>(v[2]));
        return new BinaryOp(v[0]->str(), (Exp*)v[1], (Exp*)v[2]);
      }
    private:
      std::string op_;
      Exp* right_;
    ...
    public:
      ...
      static sphingid::ast::Node* make(std::vector<sphingid::ast::Node*> v)
      {
        ...
        return new Prototype(v[0]->str(), args);
      }
    private:
      std::string name_;
    ...
    public:
      ...
      static sphingid::ast::Node* make(std::vector<sphingid::ast::Node*> v)
      {
        assert(v.size() == 3);
        // v[0] "def"
        Prototype* p = dynamic_cast<Prototype*>(v[1]);
        Exp* q = dynamic_cast<Exp*>(v[2]);
        assert(p && q);
